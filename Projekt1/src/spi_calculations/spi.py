...
#     grouped_data = df.groupby(['Kod stacji', 'Rok', 'Miesiąc']).agg({'Suma dobowa opadów [mm]': 'sum'})
#     print(grouped_data)
...
def spi_1(df):
    df = df.set_index('Kod stacji')
    grouped_data = df.groupby(['Kod stacji', 'Rok', 'Miesiąc']).agg({'Suma dobowa opadów [mm]': 'sum'})
    grouped_data = grouped_data[grouped_data['Suma dobowa opadów [mm]'] > 0]  # filtracja poniewaz wymaga tego metoda fit
    # print(grouped_data)
    alpha, loc, beta = sp.stats.gamma.fit(grouped_data, floc=0)
    # print(alpha, loc, beta)
    cdf = sp.stats.gamma.cdf(grouped_data, alpha, loc=loc, scale=beta)
...
def spi_3(df):
    df = df.set_index('Kod stacji')
    grouped_data = df.groupby(['Kod stacji', 'Rok', 'Miesiąc']).agg({'Suma dobowa opadów [mm]': 'sum'})
    grouped_data = grouped_data[grouped_data['Suma dobowa opadów [mm]'] > 0]
